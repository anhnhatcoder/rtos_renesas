...
#if 1
StaticSemaphore_t g_sensor_mutex_memory;
#endif
...
#if 0
                #if 1
                xSemaphoreCreateRecursiveMutexStatic(&g_sensor_mutex_memory);
                #else
                xSemaphoreCreateRecursiveMutex();
                #endif
                #else
#if 1
            xSemaphoreCreateMutexStatic (&g_sensor_mutex_memory);
#else
                xSemaphoreCreateMutex();
                #endif
#endif
    if (NULL == g_sensor_mutex)
    {
        rtos_startup_err_callback (g_sensor_mutex, 0);
    }
}
